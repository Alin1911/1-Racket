...
(define (queue-empty? q)
  (if (and (= 0 (queue-size-l q))(= 0 (queue-size-r q)))#t
      #f
      ))
...
(define (rotate left right Acc)
  (cond
    ...
    ((and (null? right) (not (stream-empty? left))((rotate empty-stream  right (stream-append left Acc)))
    (else  (rotate left   (cdr right) (stream-cons (car right) Acc)))
    )
)
...
(define (enqueue x q)
    ...
    [(queue left right size-l size-r)
     (if (>= size-l (+ 1 size-r)) 
         (struct-copy queue q [left left] [right (reverse (append (reverse right) (list x)))] [size-l size-l] [size-r (+ 1 size-r)])
         (struct-copy queue q [left (rotate left (reverse (append (reverse right) (list x))) empty-stream)] [right '()] [size-l (+ (+ size-l size-r) 1)] [size-r 0])
         )
...
(define (dequeue q)
  (match q
    [(queue left right size-l size-r)
     (if (>= (- size-l 1) size-r) 
         (struct-copy queue q [left (stream-rest left)] [right right] [size-l (- size-l 1)] [size-r size-r])
         (struct-copy queue q [left (rotate (stream-rest left) right empty-stream)] [right '()] [size-l (- (+ size-l size-r) 1)] [size-r 0])
         )
         ]
      )
  )
